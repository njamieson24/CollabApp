...
app.get("/highest-ranked-song/:roomCode", (req, res) => {
  ...
  // Fetch all songs in a party room, ordered by ranking (likes - dislikes)
// Fetch all songs in a party room, ordered by ranking
app.get("/songs-by-ranking/:roomCode", (req, res) => {
    const roomCode = req.params.roomCode;
  
    // Query to get all songs for the given room code ordered by ranking (likes - dislikes)
    const query = `
      SELECT song_name, ranking
      FROM songs
      WHERE room_code = ?
      ORDER BY ranking DESC
    `;
