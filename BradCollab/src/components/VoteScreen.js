...
const VoteScreen = () => {
  ...
  const handleVote = async (songId, voteType) => {
    ...
    try {
      // If the user has voted before, we need to remove the previous vote first
      const currentVote = userVotes[songId];
      if (currentVote) {
        // Send a request to remove the previous vote (whether it's up or down)
        await fetch("http://localhost:5000/remove-vote", {
          method: "POST",
          headers: {
